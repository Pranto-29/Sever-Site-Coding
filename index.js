...
async function run() {
  try {
    ...
    app.get("/search", async (req, res) => {
      try {
        let searchText = req.query.search;
        if (!searchText) return res.status(400).send({ success: false, message: "Search text is required" });

        const regex = new RegExp(searchText.trim(), "i");
        const foods = await foodCollection.find({ title: regex }).toArray();
    ...
    app.get("/foods", async (req, res) => {
      try {
        ...
        const foods = await foodCollection.find({ email }).toArray();
        res.send({ success: true, data: foods });
      } catch (err) {
        console.error(err);
        res.status(500).send({ success: false, message: "Server error" });
    ...
    app.put("/foods/:id", async (req, res) => {
      try {
        ...
        const result = await foodCollection.updateOne({ _id: new ObjectId(id) }, { $set: data });
        if (result.matchedCount === 0) return res.status(404).send({ success: false, message: "Food not found" });

        const updatedFood = await foodCollection.findOne({ _id: new ObjectId(id) });
        res.send({ success: true, data: updatedFood });
      } catch (err) {
        console.error(err);
        res.status(500).send({ success: false, message: "Server error" });
    ...
    // client.close(); // keep open for server
  }
}
